...
import { LineChart, Play, Activity, AlertCircle, DollarSign, TrendingUp, Building2, Scale, Landmark, Download, Printer } from 'lucide-react';
...
export default function ValuationEngine() {
  ...
  const handleRunModel = async () => {
    ...
    try {
      ...
      setResults({
        projections, cumulativePvFcf, terminalValue, pvTerminalValue, wacc, waccDetails,
  ...
  const handleExportCSV = () => {
    ...
    const rows = results.projections.map(p => [ `Year ${p.year}`, p.revenue.toFixed(2), p.ebitda.toFixed(2), p.nopat.toFixed(2), p.fcf.toFixed(2), p.pvFcf.toFixed(2) ]);
  ...
  return (
    <div className="max-w-7xl mx-auto py-8 px-4 sm:px-6 lg:px-8 font-sans print:py-0 print:px-0">
      <div className="mb-8 flex items-center justify-between border-b border-slate-200 pb-5 print:hidden">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 flex items-center gap-2"><LineChart className="text-[#002D72]" /> Valuation Dashboard</h1>
          <p className="text-slate-500 mt-1 text-sm">Standalone DCF valuation and calculated Cost of Capital.</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-8 flex flex-col md:flex-row items-end gap-4 print:hidden">
        ...
        <button onClick={handleRunModel} disabled={loading || !selectedEntity} className="w-full md:w-auto bg-[#002D72] hover:bg-[#001f4d] text-[#C5A059] px-8 py-2.5 rounded-lg font-bold flex items-center justify-center gap-2 transition-colors shadow-sm disabled:opacity-70 border border-[#C5A059]/30">
          {loading ? <Activity className="animate-spin" size={18} /> : <Play size={18} fill="currentColor" />} {loading ? 'Computing...' : 'Run Valuation'}
        </button>
      </div>
      ...
      {results && (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="hidden print:block mb-8 text-center border-b-2 border-slate-800 pb-4">
            ...
            <p className="text-sm font-bold text-slate-500 mt-2 uppercase tracking-widest">DCF Valuation Report</p>
          </div>

          {/* WACC Glass Box (Standalone) */}
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 relative overflow-hidden mb-8 md:w-1/2 print:border-slate-300 print:shadow-none print:w-full">
